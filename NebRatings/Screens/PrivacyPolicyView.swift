    ...
    var body: some View {
        ZStack {
            if let error = loadError {
                ...
                WebViewRepresentable(isLoading: $isLoading, loadError: $loadError)
...
struct WebViewRepresentable: UIViewRepresentable {
    ...
    func loadHTMLContent(in webView: WKWebView) {
        ...
        if let path = Bundle.main.path(forResource: "privacyPolicy", ofType: "html", inDirectory: "LegalDocs") {
        ...
        if htmlString == nil, let path = Bundle.main.path(forResource: "privacyPolicy", ofType: "html") {
            htmlString = try? String(contentsOfFile: path, encoding: .utf8)
            baseURL = Bundle.main.bundleURL
        }
        
        guard let htmlString = htmlString else {
            loadError = NSError(domain: "PrivacyPolicyView", code: -1, userInfo: [NSLocalizedDescriptionKey: "Privacy policy file not found in app bundle"])
            isLoading = false
            return
        }
